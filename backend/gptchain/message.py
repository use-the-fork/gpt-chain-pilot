...
from gptchain.element import ElementBased
from gptchain.logger import logger
from gptchain.prompt import Prompt
...
class MessageBase(ABC):
    ...
    async def _persist_create(self, message: MessageDict):
        if not gptchain_client or self.persisted:
            return

        try:
            persisted_id = await gptchain_client.create_message(message)
    ...
    async def _persist_update(self, message: MessageDict):
        if not gptchain_client or not self.id:
            return

        try:
            await gptchain_client.update_message(self.id, message)
        except Exception as e:
            if self.fail_on_persist_error:
                raise e
    ...
    async def remove(self):
        ...
        if gptchain_client and self.id:
            await gptchain_client.delete_message(self.id)
