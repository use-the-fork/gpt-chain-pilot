...
class gptchainTelemetry:
    ...
    def _build_resource(
        ...
        if config.gptchain_prod_url:
            host_name = config.gptchain_prod_url
    ...
    def configure_tracer(self):
        ...
        tracer = get_tracer("gptchain", __version__)
        return tracer
...
gptchain_telemetry = gptchainTelemetry()


def trace_event(event_name):
    if config.project.enable_telemetry:
        with gptchain_telemetry.tracer.start_as_current_span(
