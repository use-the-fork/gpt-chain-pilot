...
from gptchain.auth import get_current_user, require_login
from gptchain.client.cloud import MessageDict
from gptchain.config import config
from gptchain.context import init_ws_context
from gptchain.data import gptchain_client
from gptchain.logger import logger
from gptchain.message import ErrorMessage, Message
from gptchain.server import socket
from gptchain.session import WebsocketSession
from gptchain.telemetry import trace_event
from gptchain.types import UIMessagePayload
from gptchain.user_session import user_sessions
...
async def connect(sid, environ, auth):
    ...
    session_id = environ.get("HTTP_X_gptchain_SESSION_ID")
    ...
    WebsocketSession(
        ...
        user=user,
        token=token,
        chat_profile=environ.get("HTTP_X_gptchain_CHAT_PROFILE"),
    )
...
async def disconnect(sid):
    ...
    if gptchain_client and session and not session.has_user_message:
        if session.conversation_id:
            await gptchain_client.delete_conversation(session.conversation_id)
