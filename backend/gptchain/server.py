...
from gptchain.oauth_providers import get_oauth_provider
...
from gptchain.auth import create_jwt, get_configuration, get_current_user
...
from gptchain.data.acl import is_conversation_author
from gptchain.logger import logger
from gptchain.markdown import get_markdown_str
from gptchain.playground.config import get_llm_providers
from gptchain.telemetry import trace_event
from gptchain.types import (
    CompletionRequest,
...
app.mount(
    "/assets",
    StaticFiles(
        packages=[("gptchain", os.path.join(build_dir, "assets"))],
...
def get_html_template():
    ...
    default_url = "https://github.com/gptchain/gptchain"
    ...
    <meta property="og:image" content="https://gptchain-cloud.s3.eu-west-3.amazonaws.com/logo/gptchain_banner.png">
...
def get_user_facing_url(url: URL):
    ...
    gptchain_url = os.environ.get("gptchain_URL")
    ...
    config_url = URL(gptchain_url).replace(
        query="",
        fragment="",
    )
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if gptchain_client:
        await gptchain_client.create_app_user(app_user=app_user)
    return {
...
async def update_feedback(
    ...
    await gptchain_client.set_human_feedback(
...
async def get_user_conversations(
    ...
    res = await gptchain_client.get_conversations(payload.pagination, payload.filter)
...
async def get_conversation(
    ...
    res = await gptchain_client.get_conversation(conversation_id)
...
async def get_conversation_element(
    ...
    res = await gptchain_client.get_element(conversation_id, element_id)
    return JSONResponse(content=res)
...
async def delete_conversation(
    ...
    await gptchain_client.delete_conversation(conversation_id)
    return JSONResponse(content={"success": True})
