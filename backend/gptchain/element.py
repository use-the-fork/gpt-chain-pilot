...
import aiofiles
import filetype
from gptchain.client.base import ElementDict, ElementDisplay, ElementSize, ElementType
...
class Element:
    ...
    async def persist(self, client: gptchainCloudClient) -> Optional[ElementDict]:
    ...
    async def _persist(self, element: ElementDict):
        ...
        try:
            if self.persisted:
                await gptchain_client.update_element(element)
            else:
                await gptchain_client.create_element(element)
                self.persisted = True
    ...
    async def send(self, for_id: Optional[str] = None):
        ...
        if gptchain_client:
            element_dict = await self.persist(gptchain_client)
            if element_dict:
