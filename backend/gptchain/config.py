...
if TYPE_CHECKING:
    ...
    from gptchain.client.base import AppUser
    from gptchain.types import ChatProfile
...
config_dir = os.path.join(APP_ROOT, ".gptchain")
...
# Follow symlink for asset mount (see https://github.com/gptchain/gptchain/issues/317)
...
gptchain_prod_url = os.environ.get("gptchain_PROD_URL")
...
@dataclass()
class gptchainConfig:
    # Directory where the gptchain project is located
    root = APP_ROOT
    # gptchain server URL. Used only for cloud features
    gptchain_server: str
    # Whether or not a gptchain api key has been provided
    data_persistence: bool
    # The url of the deployed app. Only set if the app is deployed.
    gptchain_prod_url = gptchain_prod_url
...
def load_config():
    ...
    gptchain_server = os.environ.get("gptchain_SERVER", "https://cloud.gptchain.io")
    data_persistence = "gptchain_API_KEY" in os.environ

    config = gptchainConfig(
        gptchain_server=gptchain_server,
