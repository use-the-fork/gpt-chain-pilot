...
import jwt
from gptchain.client.cloud import AppUser
...
def get_jwt_secret():
    return os.environ.get("gptchain_AUTH_SECRET")


def ensure_jwt_secret():
    if require_login() and get_jwt_secret() is None:
        raise ValueError(
            "You must provide a JWT secret in the environment to use password authentication. Run `gptchain create-secret` to generate one."
        )
...
async def authenticate_user(token: str = Depends(reuseable_oauth)):
    ...
    if gptchain_client:
        try:
            persisted_app_user = await gptchain_client.get_app_user(app_user.username)
        except Exception as e:
